...
export const addRoute = (app: Express) => {
    app.post(path, validateInput(Input), async (req: Request<Input>, res: Response) => {
        const { discount, endDateTimestamp, courseId, startDateTimestamp } = req.body;

        const client = req.session.data;

        if (client.userType !== "instructor") return res.status(400).send({ error: "unauthorized" });

        const [course, user] = await Promise.all([Course.findById(courseId), User.findOne({ username: client.username })]);

        if (!course) return res.status(400).send({ error: "course not found" });
        if (!user) return res.status(400).send({ error: "user not found" });

        if (course.instructor !== user?._id) return res.status(400).send({ error: "unauthorized" });

        const startDate = new Date(startDateTimestamp);
        const endDate = new Date(endDateTimestamp);

        await Course.findOneAndUpdate({ _id: courseId, discount: { rate: discount, startDate, endDate } });
