...
export const addRoute = (app: Express) => {
    app.post(path, validateInput(Input), async (req: Request<Input>, res: Response) => {
        ...
        if (userSession.userType !== UserTypes.individualTrainee) return res.status(401).send("Unauthorized");

        const enrollment = await Enrollment.findById(enrollmentId);
        if (!enrollment) return res.status(404).send("Enrollment not found");
        if (enrollment.status !== "accepted") return res.status(400).send("Enrollment not accepted");
        ...
        if (touchedSectionsCount < sections.length / 2) {
            ...
            try {
                const updateEnrollment = await Enrollment.updateOne({ _id: enrollmentId, status: "accepted" }, { status: "refunded" }, { session });
                if (updateEnrollment.modifiedCount === 0) throw new Error("Enrollment not found or already refunded");
                await User.updateOne({ _id: user.id }, { $inc: { wallet: enrollment.amountPaid } });
