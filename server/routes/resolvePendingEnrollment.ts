...
export const addRoute = (app: Express) => {
    app.get(path, validateInput(Input), async (req: Request<Input>, res: Response) => {
        ...
        const status = accepted ? EnrollmentStatusType.accepted : EnrollmentStatusType.rejected;
        ...
        if (enrollment.matchedCount === 0) return res.status(404).send("Enrollment not found");
