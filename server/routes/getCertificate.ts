...
export const addRoute = (app: Express) => {
    app.get(path, validateInput(Input), async (req: Request<Input>, res: Response) => {
        ...
        if (sections.length !== enrollment.completedSections.length) return res.status(404).send("Enrollment not complete");
        ...
        const doc = generateCompletionCertificationDocument(studentName, courseName);

        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", `attachment; filename="${courseName} Completion Certificate.pdf"`);
        doc.pipe(res);
        doc.end();
    });
};
