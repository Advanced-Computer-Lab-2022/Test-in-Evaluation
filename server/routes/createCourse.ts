    ...
    exercises: Array(
      Record({
        ...
        (excercise) =>
          ...
          typeof excercise.answers[excercise.correctAnswer] === "string"
      )
    ),
  }),
});
...
export const addRoute = (app: Express) => {
  app.post(
    ...
    async (req: Request<Input>, res: Response) => {
      ...
      const { price, sections, subject, summary, title, videoPreviewUrl } =
        req.body;
      ...
      await Promise.all(
        sections.map(({ description, title, totalHours, videoUrl, exam }) =>
          Section.create({
            description,
            name: title,
